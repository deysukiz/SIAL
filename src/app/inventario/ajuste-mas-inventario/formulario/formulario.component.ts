...
export class FormularioComponent {
  ...
  */
  ...
  ngOnInit() {
    ...
    this.pdfworker = new Worker('web-workers/inventario/ajuste-mas.js');
          ...
          for (let l of ctrlLotes.controls['lotes'].controls) {
            ...
            if (l.controls.id.value === item.id) {
              ...
              if (cantidad_lote <= l.controls.existencia.value) {
                let existencia_minima = Number(l.controls.existencia.value) + Number(1);
                this.notificacion.alert('Cantidad Inválida', 'La cantidad mínima es: ' + existencia_minima, objeto);
  ...
  validar_cantidad_lote(i, val, i2, modo_salida) {
    if (modo_salida === 'N') {
      if (val.controls.cantidad.value <= val.controls.existencia.value) {
        ...
        let existencia_minima = Number(val.controls.existencia.value) + Number(1);
        this.notificacion.alert('Cantidad Inválida', 'La cantidad mínima es: ' + existencia_minima, objeto);
