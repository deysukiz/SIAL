...
export class FormularioComponent implements OnInit {
  ...
  lista_proveedores= [];
  /**
   * Contiene la lista de almacenes
   * @type {any}
   */
  lista_almacenes= [];
  /**
   * Contiene la lista de CLUES
   * @type {any}
   */
  lista_clues= [];

  /**
   * Variable que muestra un texto de acuerdo a la lista de almacenes.
   */
  mensaje_almacen: String;
  ...
  // Harima: para ver si el formulario es para crear o para editar
  /**
   * Título que se muestra en el formulario 'Nuevo' o 'Editar'
   */
  formularioTitulo: string = 'Nuevo';
  /**
   * Contiene un valor true cuando es un borrador.
   */
  esEditar: boolean = false;

  // Harima: Estos totales solo te toman en cuenta cuando el pedido ya estuvo en Por Surtir,
  // y recibio insumos, pero por alguna razon se regreso a Borrador, es para ajustar el presupuesto disponible
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      ...
      if(params['id']){
        ...
        this.pedidosService.ver(params['id']).subscribe(
          pedido => {
            ...
            if(fecha_pedido[0] < now.getFullYear()) {
              this.fechasValidas.push( {
                fecha: fecha_pedido[0] + '-' + fecha_pedido[1] + '-' + fecha_pedido[2],
              ...
              this.fechasValidas.push(
                {
                  ...
                  // descripcion: this.meses[mes_actual] + ' ' + now.getFullYear()
                }); // fecha actual
            }
                ...
                this.fechasValidas.push(
                  {
                    ...
                    // descripcion: this.meses[mes] + ' ' + anio
                  }); // fecha siguiente
              }
            }
            ...
            for(let i in pedido.insumos){
              ...
              if(pedido.tipo_pedido_id != 'PJS'){
                ...
                for( let j in insumo.lista_clues){
        ...
        this.fechasValidas.push(
          {
            fecha: now.getFullYear() + '-' + (month) + '-' + (day),
            // descripcion: this.meses[mes] + ' ' + now.getFullYear()
          }); // fecha actual
        // }
    ...
    // this.pedidos[0].observaciones = null;// si es nuevo poner la fecha actual si no poner la fecha con que se guardo
    // inicializar el data picker minimo y maximo
    let date = new Date(); 
    if (!this.pedido.datos.get('fecha_movimiento').value) {
      this.fecha_actual = date.getFullYear() + '-' + ('00' + (date.getMonth() + 1)).slice(-2) + '-' + date.getDate();
      this.pedido.datos.get('fecha_movimiento').patchValue(this.fecha_actual);
      console.log(this.pedido.datos);
    } else {
      this.fecha_actual = this.pedido.datos.get('fecha_movimiento').value;
    }

    console.log(this.pedido);
    this.cargarCatalogo('programa', 'lista_programas', 'estatus');
    this.cargarCatalogo('proveedores', 'lista_proveedores', 'activo');
    this.cargarCatalogo('unidades-medicas', 'lista_clues');
    // this.cargarCatalogo('unidades-medicas', 'lista_almacenes');
  } /** FIN ngOnInit */
   ...
   */
  cargarCatalogo(url, modelo, estado: any = null) {
    console.log(this.pedido.tieneError('clues_destino', 'required'));
    this[modelo] = [];
  ...
  /**
   * Metodo que nos ayuda a llenar la lista de almacenes de la CLUES seleccionada.
   */
  llenarCatalogoDependiente() {
    for (let item of this.lista_clues) {
      if (item.clues === this.pedido.datos.value.clues_destino) {
        this.lista_almacenes = item.almacenes;
        if (this.lista_almacenes.length > 0) {
          this.mensaje_almacen = '';
        } else {
          this.mensaje_almacen = 'Esta CLUES no tiene almacenes disponibles. ';
        }
        break;
      }
    }
    console.log(this.pedido.datos.value);
    console.log(this.pedido.tieneError('clues_destino', 'required'));
  }

  /**
   * Método que ayuda a obtener la ruta de la dirección del módulo
   */
  obtenerDireccion(): string {
    // if(this.pedidos[this.pedidoActivo].status == 'AB'){
    if (this.pedido.status === 'BR') {
      return '/almacen-estandar/pedidos/borradores';
    }else{
    ...
    // ######### PEDIDOS JURISDICCIONALES #########
      ...
      for ( let i in this.pedido.lista){
        ...
        if (this.pedido.lista[i].clave == insumo.clave){
          ...
          for (let j in this.pedido.lista[i].lista_clues){
  ...
  buscar(e: KeyboardEvent, input: HTMLInputElement, inputAnterior: HTMLInputElement,  parametros: any[]){
    ...
    if (arregloResultados.length > 1 ) {
      ...
      let filtro = arregloResultados[0];
      let match: any[] = [];
  ...
  eliminarInsumo(item, index, filtro: boolean = false){
    ...
    let i = this.listaClaveAgregadas.indexOf(item.clave);
  ...
  finalizar(){
    let validacion_palabra = prompt('Atención el pedido ya no podra editarse, para confirmar que desea concluir el pedido por favor escriba: CONCLUIR PEDIDO');
  ...
  guardar(finalizar: boolean = false) {
    ...
    if (this.pedido.datos.invalid) {
      this.pedido.datos.get('fecha_movimiento').markAsTouched();
      this.guardando = false;
      this.mensajeError = new Mensaje(false);
      this.mensajeError.texto = 'Se encontaron errores en el formulario.';
    ...
    if (this.erroresEnInsumos.errores > 0){
      let insumos_errores = [];
      for (let i in this.erroresEnInsumos.lista) {
    ...
    if (this.esEditar){
      ...
      this.pedidosService.editar(this.pedido.id, guardar_pedido).subscribe(
        pedido => {
          ...
          if (pedido.status != 'BR'){
            this.router.navigate(['/almacen-estandar/pedidos/ver/' + pedido.id]);
          }
      ...
      this.pedidosService.crear(guardar_pedido).subscribe(
        pedido => {
          ...
          // console.log(pedido);
          this.router.navigate(['/almacen-estandar/pedidos/editar/' + pedido.id]);
          // hacer cosas para dejar editar
        },
        error => {
          this.guardando = false;
