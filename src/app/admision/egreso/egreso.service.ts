...
export class EgresoService {
  ...
  static readonly URL: string = "admision/paciente-egreso";
    ...
    listaCompleta(pagina:number = 1, resultados_por_pagina:number =20 ): Observable<any>{
      return this.jwtRequest.get("admision/paciente-egreso",null,{page: pagina, per_page: resultados_por_pagina}).map( (response: Response) => response.json().data);
    }

    buscarCompleta(term: string, pagina:number = 1, resultados_por_pagina:number =20 ): Observable<any>{
      return this.jwtRequest.get("admision/paciente-egreso",null,{q: term, page: pagina, per_page: resultados_por_pagina}).map( (response: Response) => response.json().data);
    }
   ...
   crear(pacienteEgreso: PacienteEgreso): Observable<PacienteEgreso> {
      return this.jwtRequest.post("admision/paciente-egreso",pacienteEgreso).map( (response: Response) => response.json().data) as Observable<PacienteEgreso>;
    }
